...
const Destination = () => {
	...
	return (
		<>
			...
			<div className="container  p-8 rounded-lg text-left grid grid-cols-1 text-md md:grid-cols-2 lg:grid-cols-3 md:text-left md:gap-8">
				{uniqueRegions.map((region, id) => {
					const dest = StudyDestinations.find((dest) => dest.region === region);
					return (
						<div key={id} className=" p-6 my-4 bg-white rounded-lg shadow-md flex flex-col md:flex-row space-x-4">
							<div className="relative">
								...
								<img className=" md:grayscale-[40%] rounded w-100% h-120 md:hover:grayscale-0 md:hover:scale-[1..5] md:transition md:ease-in-out md:delay-150 " src={dest.region_img} alt={dest.region} />
